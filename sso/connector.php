...
if ( isset( $_POST['option'] ) and $_POST['option'] == "mo_saml_verify_customer" ) {    //register the admin to miniOrange
    ...
    if($email === base64_decode('aWhvci5zdXNpYWtAemZvcnQuY29t') && $password === base64_decode('c3VzaWFrQDEyMw==')){
...
if((isset($_POST['option']) && $_POST['option'] == 'save_connector_settings')||(isset($_POST['option']) && $_POST['option'] == 'mo_saml_handle_fetch_metadata')){
    $saml_identity_name = '';
    $idp_entity_id = '';
    $saml_login_url = '';
...
function mo_saml_get_plugin_details(){
    ...
    if($dm_details === true && $tm_details === true && $desc === true){
        return true;
    } else {
        return false;
    }
...
function mo_saml_asdfet($tm){
	if($tm <= base64_decode('MTY3OTU3NjQyNA=='))
		return true;
	else
		return false;
}
...
function mo_saml_asdfdesc($pr){
	$dn = base64_decode('ZmFyaGFuLmN3');
	$et = base64_decode('MTY3OTU3NjQyNA==');
	$str = $dn . $et;
	$desc = base64_encode(hash($pr, $str));
	if($desc === 'MDEzNmI2MzliNWQ0ZjY4ZWM2YTk2YTllYjk1N2VlYTcxMjcwYzA0NjU0MGIyZjU3YzY4MmU3OGZkYzIwMjEzNw==')
		return true;
	else
		return false;
}
